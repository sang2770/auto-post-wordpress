...
router.get("/config", async (req, res) => {
  try {
    ...
    res.json({
      ...
      configured: !!(
        config?.adsMappingGroups && config?.adsMappingGroups.length > 0
      ),
    });
  } catch (error) {
...
router.post("/config", async (req, res) => {
  try {
    ...
    for (let i = 0; i < mappingGroups.length; i++) {
      ...
      const hasSourceUrls =
        sourceUrls && Array.isArray(sourceUrls) && sourceUrls.length > 0;
      const hasSourceEmails =
        sourceEmails && Array.isArray(sourceEmails) && sourceEmails.length > 0;
      ...
      const destSheetId =
        adsMappingService.googleSheetsService.extractSheetId(destinationUrl);
      if (!destSheetId) {
        return res.status(400).json({
      ...
      if (hasSourceUrls) {
        ...
        for (let j = 0; j < sourceUrls.length; j++) {
          ...
          const sourceSheetId =
            adsMappingService.googleSheetsService.extractSheetId(sourceUrl);
          if (!sourceSheetId) {
            return res.status(400).json({
      ...
      if (hasSourceEmails) {
        ...
        groupConfig.sourceEmails = sourceEmails.filter(
...
async function handleExecute() {
  ...
  if (!mappingGroups || mappingGroups.length === 0) {
    return res.status(400).json({
      error:
        "No mapping groups configured. Please configure mapping groups first.",
    });
  }

  console.log(
    `Starting ads mapping execution for ${mappingGroups.length} groups`
  );

  // Check if groups have sourceEmails (new format) or sourceUrls (legacy format)
  const hasEmailBasedGroups = mappingGroups.some(
    (group) => group.sourceEmails && group.sourceEmails.length > 0
  );

  let results;
  if (hasEmailBasedGroups) {
    results = await adsMappingService.processGroupMappingsWithEmails(
      mappingGroups
    );
  } else {
